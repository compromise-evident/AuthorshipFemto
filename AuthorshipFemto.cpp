...
/* Version 1.0.1, 50-char multi-way functions. This is a minuscule and extremely
...
verifiable authorized-only number modification.) Documentation has been reduced:
github.com/compromise-evident/AuthorshipFemto   See Authorship for more details.
...
{	bool bulk_verification_efficiency = false; /* DEFAULT=FALSE ELSE NO PLAINTEXT FILES.       Octet if broken >     |
	...
	{	//Checks if files already exists.
		...
		//Number of possible keys (95^50 or 10^99) ~= 769449752767133292742943790958350921451790972423891374406296339660826788531267084181308746337890625
	...
	{	/* Basic layout:
		...
		out_stream << "AuthorshipFemto.public v1.0.1=\r\n\r\n"; //34-byte header must be. Use "\r\n" for all returns (for max compatibility.)
	...
	{	/* Basic layout:
		...
		in_stream.open("AuthorshipFemto.public");
		char temp_file_byte; for(int a = 0; a < 34; a++) {in_stream.get(temp_file_byte);} //As the 34-byte headers can change, they are skipped.
		...
		{	for(int b = 0; b < 50; b++) //Constructively applies randomness based on keys[] (used as seeds, 50 items per actual_key[].)
			...
			//Writes the result back to keys[].
			for(int d = 0; d < 50; d++)
