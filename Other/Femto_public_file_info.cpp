/* Prints old number, new number, and message of AuthorshipFemto.public files.
...
{	bool bulk_verification_efficiency = false; /* DEFAULT=FALSE ELSE NO PLAINTEXT FILES.       Octet if broken >     |
	...
	cout << "\nDrag & drop AuthorshipFemto.public file into terminal or enter path:\n\n";
	...
	for(int a = 0; a < 1008; a++) //Grabbing 1,568 50-character items (1,008 functions + 560 keys. Here, keys are skipped for efficiency.)
	{	//Gets function.
		for(int b = 0; b < 50; b++)
		...
		if(temp_file_byte == ' ')
		{	b[a] = 1; //1 = key present.
			for(int c = 0; c < 50; c++) {in_stream.get(temp_file_byte);}
